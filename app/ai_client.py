...
class AIClient:
    ...
    def detect_chat_action(self, *, user_message: str, default_project: str, default_issue_type: str) -> dict[str, Any]:
        instructions = (
            ...
            "9) If user asks to onboard someone, prepare a new employee, or make an odovzdavaci/handover protocol for giving a device to a person -> action=onboarding.\n"
            "10) If user asks to offboard someone, end-of-contract, preberaci/return protocol, vratenie zariadenia, or prepare a return document -> action=offboarding.\n"
            "10b) If user asks print protocol for a specific Assets object key like CDX-4 -> action=assets_print.\n"
